...
## G. NEW FEATURES ADDED (Post-Review)

### 1. Environment Doctor Tool (`godot_env_doctor`)
...
Checks:
✅ Godot Executable - version detection, path validation
✅ Project Path - validates project.godot exists
✅ LSP Connection - tests port 6005 connectivity
✅ GDToolkit - gdlint, gdformat, gdparse availability
✅ GdUnit4 - addon detection
✅ Node.js - version compatibility

Provides actionable remediation steps for any issues found.
```

### 2. Comprehensive Shader Support (6 new tools)

| Tool | Description |
|------|-------------|
| `godot_analyze_shader` | Deep analysis - uniforms, varyings, functions, complexity |
| `godot_lint_shader` | Lint for errors, deprecations, performance issues |
| `godot_lint_all_shaders` | Project-wide shader linting |
| `godot_shader_performance` | Performance score, grade, optimization tips |
| `godot_find_shaders` | Find all shaders with summary stats |
| `godot_get_shader_docs` | Built-in uniforms, render modes by shader type |

**Shader Linting Catches:**
- Deprecated Godot 3→4 functions (texture2D → texture)
- SCREEN_TEXTURE usage without hints
- Performance issues (discard, branching, pow())
- Magic numbers
- Missing shader_type declaration
- Complexity metrics (texture samples, branching, loops)
...
*Updated with new features on 2026-01-06*
